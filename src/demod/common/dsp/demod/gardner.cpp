...
namespace dsp {
    namespace clock_recovery {
        ...
        void Gardner::init(stream<complex_t>* in, double omega, double omegaGain, double muGain, double omegaRelLimit, int outSps, int interpPhaseCount, int interpTapCount) {
            _omega = omega;
            _outSps = outSps;
            _spsctr = 0;
        ...
        void Gardner::setOmega(double omega) {
        ...
        void Gardner::setOmegaGain(double omegaGain) {
            assert(base_type::_block_init);
            std::lock_guard<std::recursive_mutex> lck(base_type::ctrlMtx);
            _omegaGain = omegaGain;
        ...
        void Gardner::setMuGain(double muGain) {
            assert(base_type::_block_init);
            std::lock_guard<std::recursive_mutex> lck(base_type::ctrlMtx);
            _muGain = muGain;
            pcl.setCoefficients(_muGain, _omegaGain);
        ...
        void Gardner::setOmegaRelLimit(double omegaRelLimit) {
            assert(base_type::_block_init);
            std::lock_guard<std::recursive_mutex> lck(base_type::ctrlMtx);
            _omegaRelLimit = omegaRelLimit;
            pcl.setFreqLimits(_omega * (1.0 - _omegaRelLimit), _omega * (1.0 + _omegaRelLimit));
        ...
        void Gardner::setInterpParams(int interpPhaseCount, int interpTapCount) {
            assert(base_type::_block_init);
            std::lock_guard<std::recursive_mutex> lck(base_type::ctrlMtx);
            base_type::tempStop();
            _interpPhaseCount = interpPhaseCount;
        ...
        int Gardner::process(int count, const complex_t* in, complex_t* out) {
        ...
        void Gardner::generateInterpTaps() {
